...
- `skipFederatedStats` – skips generation of `federated-stats.json` to reduce build time. Enable it only for CSR-only applications when neither `flushChunks()` nor any downstream host or custom tooling consumes that asset.
